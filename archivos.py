...
def agregar_archivo(numero, nombre, grupo_id, creado_por=None):
    conn = get_db()
    cur = conn.cursor()
    ...
    cur.execute(
        ...
        RETURNING id
        """,
        (numero, nombre, caja_id, creado_por, grupo_id)
    )
    ...
    registrar_movimiento(
        ...
        datos_despues={"id": archivo_id, "numero": numero, "nombre": nombre, "caja_id": caja_id},
    )


def modificar_archivo(numero, nombre, grupo_id, usuario_id=None):
    conn = get_db()
    cur = conn.cursor()

    cur.execute(
        "SELECT id, nombre FROM archivos WHERE numero = %s AND grupo_id = %s",
        (numero, grupo_id)
    )
    antes = cur.fetchone()
...
def eliminar_archivo(numero, grupo_id, usuario_id=None):
    ...
    cur.execute(
        "SELECT id, numero, nombre, caja_id, pdf_path FROM archivos WHERE numero = %s AND grupo_id = %s",
        (numero, grupo_id)
    )
    antes = cur.fetchone()
